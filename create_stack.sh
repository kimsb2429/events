...
echo $ip
response=$(curl --write-out '%{http_code}' --silent --output /dev/null http://107.23.254.55:8080)
...
process_group_id=$(curl http://107.23.254.55:8080/nifi-api/flow/process-groups/root | jq -r '.processGroupFlow.id')
